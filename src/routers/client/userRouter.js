...
router.get("/study_history", (req, resp) => {
    ...
    resp.tool.execSQLTEMPAutoResponse(`
    SELECT
	t_study_course.user_id,
	t_study_course.course_id,
	t_study_course.course_title,
	t_study_course.course_fm_url,
	t_study_course.course_is_hot,
	t_study_course.learned_count,
...
GROUP BY
	t_course_outline.course_id,t_study_course.learned_count, t_study_course.course_is_hot, t_study_course.course_fm_url, t_study_course.course_title, t_study_course.course_id, t_study_course.user_id;
    `, "获取学习记录成功!")
        ...
        if (result.length > 0) {
            ...
            `, [newPath, user_id]).then(result=>{
                if (result.affectedRows > 0) {
                    ...
                    resp.send(resp.tool.ResponseTemp(-1, "更新头像失败", {}))
                }
...
router.post("/update_info", (req, resp) => {
    ...
    `, [nick_name, intro, user_id]).then(result=>{
        ...
        // id 存在 新旧内容不一样
        // id 存在  新旧内容一样
        // affectedRows:条件符合的前提下，能够查询到的行数
        // changedRows:新旧内容一样时为0
        // id不存在并不会报错
        if (result.affectedRows > 0) {
            // 更新成功，并返回给客户端
            resp.tool.execSQL("select id, account, nick_name, header, intro from t_user where id=?;", [user_id]).then(userResult=>{
